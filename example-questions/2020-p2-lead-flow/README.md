...
**從 $x_2$ 方程式**:
$$
k_{21}\, x_1^\ast = (k_{02}+k_{12})\, x_2^\ast \;\Rightarrow\; x_2^\ast = \frac{k_{21}}{k_{02}+k_{12}}\, x_1^\ast
$$

**從 $x_3$ 方程式**:
$$
k_{31}\, x_1^\ast = k_{13}\, x_3^\ast \;\Rightarrow\; x_3^\ast = \frac{k_{31}}{k_{13}}\, x_1^\ast
$$

代入 $x_1$ 方程式整理後:

$$
x_1^\ast = \frac{I_1}{k_{01} + \dfrac{k_{21}\,k_{02}}{k_{02}+k_{12}}}
$$

**數值**(代入):
$$
x_1^\ast \approx \frac{49.3}{0.0211 + 0.0111 \cdot \frac{0.0162}{0.0286}} \approx \frac{49.3}{0.02738} \approx 1800\,\mu\text{g}
$$

$$
x_2^\ast = \frac{0.0111}{0.0286} \cdot 1800 \approx 699\,\mu\text{g}
$$

$$
x_3^\ast = \frac{0.0039}{0.000035} \cdot 1800 \approx \boxed{200{,}640\,\mu\text{g}}
$$

**注意 $x_3^\ast$ 是 $x_1^\ast$ 的 111 倍**——骨骼是巨大的儲存槽。
...
- **$x_1$**:幾乎已經是穩態(1800 ≈ $x_1^\ast$),所以變化不大。
...
**在平衡時**:$I_1 = k_{01}x_1^\ast + k_{02}x_2^\ast$
...
1. **先解析地把 $x_1^\ast, x_2^\ast, x_3^\ast$ 列出來**——這幫你預測模擬會看到什麼。
2. **發現 $k_{13}$ 比其他都小三個數量級**——這是 paper key insight,寫下來。
3. **800 vs 8000 天的對比**:強調**血液/組織快、骨骼慢**。不是兩個一樣的圖。
4. **兩組 IC 對比**:IC2 給的就是 $x_1^\ast$,所以 $x_1$ 不動;但 IC2 給的 $x_2 = 1800$ **不是** $x_2^\ast=699$,所以 $x_2$ 會掉下來。$x_3$ 兩種 IC 行為很不同(0 vs 1800 起點,終點都是 200,640)。
