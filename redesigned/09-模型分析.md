...
$$
(V^*, P^*) = (0, 0) \quad\text{以及}\quad (V^*, P^*) = \left(\dfrac{d}{cb},\;\dfrac{a}{b}\right)
$$
...
**(關鍵想法)** 在平衡點 $X^*$ 附近,把每個變數寫成「平衡點 + 小擾動」:$X = X^* + x$。對非線性方程 $dX/dt = f(X)$ 做一階 Taylor:

$$
\dfrac{dx}{dt} \;\approx\; \left.\dfrac{\partial f}{\partial x}\right|_{X^*}\!\cdot x
$$
